    ...
    def download_embedder(embedders_mode, hubert):
        ...
        if embedders_mode != "transformers" and not os.path.exists(model_path): 
            if embedders_mode == "whisper":
                huggingface.HF_download_file("".join([codecs.decode("uggcf://uhttvatsnpr.pb/ArbCl/IbvprPbairefvba-Zbqryf/erfbyir/znva/fcrnxre_qvnevmngvba/", "rot13"), hubert]), model_path)
            else:
...
def check_spk_diarization(model_size, speechbrain=True):
    ...
    if not os.path.exists(whisper_model): huggingface.HF_download_file("".join([codecs.decode("uggcf://uhttvatsnpr.pb/ArbCl/IbvprPbairefvba-Zbqryf/erfbyir/znva/fcrnxre_qvnevmngvba/", "rot13"), model_size, ".pt"]), whisper_model)
    ...
    if speechbrain:
        for f in ["classifier.ckpt", "config.json", "embedding_model.ckpt", "hyperparams.yaml", "mean_var_norm_emb.ckpt"]:
            ...
            if not os.path.exists(speechbrain_model): huggingface.HF_download_file(codecs.decode("uggcf://uhttvatsnpr.pb/ArbCl/IbvprPbairefvba-Zbqryf/erfbyir/znva/fcrnxre_qvnevmngvba/fcrrpuoenva/", "rot13") + f, speechbrain_model)
